...
class FlagsTest {
  ...
  def testTimedFlag(flag: Long, test: Symbol => Boolean, enabling: Boolean) = {
    assertTrue(withFlagMask(InitialFlags)(test(sym.setFlag(flag))) == !enabling)
    assertTrue(withFlagMask(InitialFlags | flag)(test(sym.setFlag(flag))) == enabling)
  }
  ...
  def testTimedFlags(): Unit = {
    ...
    assertTrue(withFlagMask(InitialFlags)(sym.setFlag(PRIVATE | notPRIVATE).flags & PRIVATE) == PRIVATE)
    assertTrue(withFlagMask(AllFlags)(sym.setFlag(PRIVATE | notPRIVATE).flags & PRIVATE) == 0)
  }
  ...
  def normalLateOverlap(): Unit = {
    ...
    for (i <- 0 to 3) {
      val f = 1L << i
      assertTrue(withFlagMask(AllFlags)(sym.setFlag(f << LateShift).flags & f) == 0) // not treated as late flag
    }
    for (i <- 4 to 8) {
      val f = 1L << i
      assertTrue(withFlagMask(AllFlags)(sym.setFlag(f << LateShift).flags & f) == f) // treated as late flag
    }
  }
  ...
  def normalAnti(): Unit = {
    for (i <- 0 to 2) {
      val f = 1L << i
      assertTrue(withFlagMask(AllFlags)(sym.setFlag(f | (f << AntiShift)).flags & f) == 0) // negated flags
    }
    for (i <- 3 to 7) {
      val f = 1L << i
      assertTrue(withFlagMask(AllFlags)(sym.setFlag(f | (f << AntiShift)).flags & f) == f) // not negated
    }
  }
