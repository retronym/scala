...
trait RedBlackTreeInvariants {
  ...
  object RedTree {
    def unapply[A,B](t:Tree[A,B]) = if (t.isRed) Some(t.key, t.value, t.left, t.right) else None
  }
  object BlackTree {
    def unapply[A,B](t:Tree[A,B]) = if (t.isBlack) Some(t.key, t.value, t.left, t.right) else None
  }
