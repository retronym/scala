  ...
  */
  ...
  class Request(val line: String, origTrees: List[Tree], generousImports: Boolean = false, synthetic: Boolean = false) extends ReplRequest {
    ...
    class Wrapper {
      ...
      def mkUnit(contributors: List[MemberHandler]): CompilationUnit = {
      ...
      def wrapperDef(iw: String) =
        if (isClassBased) s"sealed class $iw extends _root_.java.io.Serializable" else s"object $iw"

      import nme.{ INTERPRETER_IMPORT_WRAPPER => iw }

      /** Like postamble for an import wrapper. */
      def postwrap: String = if (isClassBased) s"val $iw = new $iw" else ""
    }


    private[interpreter] lazy val ObjectSourceCode: Wrapper = new Wrapper
    ...
    def compile: Boolean = {
      ...
      lineRep.compile(ObjectSourceCode.mkUnit(handlers)) && {
