...
abstract class Enumeration (initial: Int) extends Serializable {
  ...
  class ValueSet private[ValueSet] (private[this] var nnIds: immutable.BitSet)
    ...
    def sortedIterableFactory = immutable.SortedSet
    protected[this] def fromSpecificIterable(coll: Iterable[Value]) = ValueSet.fromSpecific(coll)
    def iterableFactory = immutable.Set
    protected[this] def newSpecificBuilder() = ValueSet.newBuilder
    protected[this] def sortedFromIterable[B : Ordering](it: Iterable[B]) = immutable.SortedSet.from(it)

    def map(f: Value => Value): ValueSet = fromSpecificIterable(View.Map(toIterable, f))
    def flatMap(f: Value => IterableOnce[Value]): ValueSet = fromSpecificIterable(View.FlatMap(toIterable, f))
  }
  ...
  object ValueSet extends SpecificIterableFactory[Value, ValueSet] {
    ...
    def newBuilder: mutable.Builder[Value, ValueSet] = new mutable.Builder[Value, ValueSet] {
      ...
      def add (x: Value) = { b += (x.id - bottomId); this }
      def clear() = b.clear()
