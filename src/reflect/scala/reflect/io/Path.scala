...
object Path {
  ...
  def apply(jfile: JFile): Path = try {
    def isFile = {
      //if (Statistics.areSomeColdStatsEnabled) Statistics.incCounter(IOStats.fileIsFileCount)
      jfile.isFile
    }

    def isDirectory = {
      //if (Statistics.areSomeColdStatsEnabled) Statistics.incCounter(IOStats.fileIsDirectoryCount)
      jfile.isDirectory
    }
...
class Path private[io] (val jfile: JFile) {
  ...
  def exists = {
    //if (Statistics.areSomeColdStatsEnabled) Statistics.incCounter(IOStats.fileExistsCount)
