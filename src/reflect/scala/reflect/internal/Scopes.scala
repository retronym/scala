...
trait Scopes extends api.Scopes { self: SymbolTable =>
  ...
  class Scope protected[Scopes]() extends ScopeApi with MemberScopeApi {
    ...
    protected[scala] def createHashIfRequired() {
  ...
  def newNestedScope(outer: Scope): Scope = {
    val nested = newScope
    initNestedScope(nested, outer)
    nested
  }
  protected def initNestedScope(nested: Scope, outer: Scope) {
    nested.elems = outer.elems
    nested.nestinglevel = outer.nestinglevel + 1
    if (outer.hashtable ne null)
      nested.hashtable = outer.hashtable.clone()
  }
