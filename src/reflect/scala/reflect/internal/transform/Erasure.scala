...
trait Erasure {
  ...
  abstract class ErasureMap extends TypeMap {
    ...
    def eraseNormalClassRef(tref: TypeRef): Type = {
      ...
      if ((pre1 eq pre) && (args eq Nil)) tref // OPT
      else typeRef(pre1, clazz, List()) // #2585
    }
    ...
    def applyInArray(tp: Type): Type = tp match {
      case tref @ TypeRef(pre, sym, args) if (sym.isDerivedValueClass) => eraseNormalClassRef(tref)
      case _ => apply(tp)
    }
  }
