...
abstract class UnPickler {
  ...
  class Scan(_bytes: Array[Byte], offset: Int, classRoot: Symbol, moduleRoot: Symbol, filename: String) extends PickleBuffer(_bytes, offset, -1) {
    ...
    protected def readType(forceProperType: Boolean = false): Type = readType(forceProperType, false)
    private def readType(forceProperType: Boolean, debug: Boolean): Type = {
      val tag = readByte()
      val end = readEnd()
      ...
      if (debug) {
        println(s"readType = $result")
        if (result.toString != "=> Map[String,p1.X]") {
          println("!! Expected info of `=> Map[String,p1.X]`")
    ...
    private class LazyTypeRef(i: Int) extends LazyType with FlagAgnosticCompleter {
      ...
      protected def completeInternal(sym: Symbol) : Unit = try {
        ...
        val debug = sym.name.string_==("mystery")
        val tp = at(i, () => readType(sym.isTerm, debug)) // after NMT_TRANSITION, revert `() => readType(sym.isTerm)` to `readType`
