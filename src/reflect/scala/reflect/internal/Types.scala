...
//      assert(sym ne NoSymbol, this)
   ...
   /** Convert toString avoiding infinite recursions by cutting off
    ...
    private def findDecl(name: Name, excludedFlags: Int): Symbol = {
      ...
      while (e ne null) {
        if (excludedFlags == 0 || !e.sym.hasFlag(excludedFlags.toLong)) {
...
// Hash consing --------------------------------------------------------------
  ...
  private val initialUniquesCapacity = 32768
...
// Helper Classes ---------------------------------------------------------
  ...
  def isRawIfWithoutArgs(sym: Symbol) = sym.isClass && !sym.hasPackageFlag && sym.isJavaDefined && !sym.typeParams.isEmpty
