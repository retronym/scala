  ...
  abstract class Type extends TypeApiImpl with Annotatable[Type] {
    ...
    */
    ...
    def asSeenFrom(pre: Type, clazz: Symbol): Type = {
      val start = if (Statistics.areSomeColdStatsEnabled) statistics.pushTimer(typeOpsStack, asSeenFromNanos)  else null
      try {
        val trivial = (
             this.isTrivial
      ...
      } finally if (Statistics.areSomeColdStatsEnabled) statistics.popTimer(typeOpsStack, start)
    }
...
//      assert(sym ne NoSymbol, this)
    ...
    def <:<(that: Type): Boolean = {
      if (Statistics.areSomeColdStatsEnabled) stat_<:<(that)
      else {
        (this eq that) ||
    ...
    def stat_<:<(that: Type): Boolean = {
      if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(subtypeCount)
      val start = if (Statistics.areSomeColdStatsEnabled) statistics.pushTimer(typeOpsStack, subtypeNanos) else null
      val result =
        (this eq that) ||
        (if (explainSwitch) explain("<:", isSubType(_: Type, _: Type), this, that)
         else isSubType(this, that))
 ...
 /** A base class for types that defer some operations
  ...
  abstract class SingletonType extends SubType with SimpleTypeProxy with SingletonTypeApi {
    ...
    override def baseTypeSeq: BaseTypeSeq = {
      if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(singletonBaseTypeSeqCount)
...
*/
    ...
    if (period != currentPeriod) {
      ...
      if (!isValidForBaseClasses(period)) {
        if (tpe.parents exists typeContainsTypeVar) {
          ...
          if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(compoundBaseTypeSeqCount)
          val start = if (Statistics.areSomeColdStatsEnabled) statistics.pushTimer(typeOpsStack, baseTypeSeqNanos) else null
          try {
  ...
  private def defineBaseClassesOfCompoundType(tpe: CompoundType, force: Boolean) {
    ...
    if (period == currentPeriod) {
      ...
      if (!isValidForBaseClasses(period)) {
        val start = if (Statistics.areSomeColdStatsEnabled) statistics.pushTimer(typeOpsStack, baseClassesNanos) else null
        try {
  ...
  protected def defineBaseTypeSeqOfTypeRef(tpe: TypeRef) = {
    ...
    if (period != currentPeriod) {
      tpe.baseTypeSeqPeriod = currentPeriod
      if (!isValidForBaseClasses(period)) {
        if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(typerefBaseTypeSeqCount)
        val start = if (Statistics.areSomeColdStatsEnabled) statistics.pushTimer(typeOpsStack, baseTypeSeqNanos) else null
        try {
          tpe.baseTypeSeqCache = undetBaseTypeSeq
...
// Hash consing --------------------------------------------------------------
  ...
  protected def unique[T <: Type](tp: T): T =  {
    if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(rawTypeCount)
