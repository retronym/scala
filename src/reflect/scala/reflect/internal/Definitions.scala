...
trait Definitions extends api.StandardDefinitions {
  ...
  abstract class DefinitionsClass extends DefinitionsApi with ValueClassDefinitions {
    ...
    def isJavaVarArgs(params: Seq[Symbol])  = (params ne Nil) && isJavaRepeatedParamType(params.last.tpe)
    def isScalaVarArgs(params: Seq[Symbol]) = (params ne Nil) && isScalaRepeatedParamType(params.last.tpe)
    def isVarArgsList(params: Seq[Symbol])  = (params ne Nil) && isRepeatedParamType(params.last.tpe)
    def isVarArgTypes(formals: Seq[Type])   = (formals ne Nil) && isRepeatedParamType(formals.last)
