...
trait BaseTypeSeqs {
  ...
  self =>
    if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(baseTypeSeqCount)
    if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(baseTypeSeqLenTotal, elems.length)
