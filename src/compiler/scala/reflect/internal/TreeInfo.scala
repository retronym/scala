...
abstract class TreeInfo {
  ...
  /** A tree corresponding to the type _root_.scala.xml.Elem
   *  which is what is created for xml literals.
   */
  private lazy val XmlElemNameSegments = nme.segments(fulltpnme.ScalaXmlElem)
  ...
  def isBareExpression(t: Tree) = t match {
    case _: Assign                          => false // we don't want to include assignments
    case _: TermTree | _: Ident | _: Select => true  // ... but do want other unnamed terms.
    case _                                  => false
  }

  def nameSegments(tree: Tree): List[Name] = tree match {
    case Ident(name)        => List(name)
    case Select(qual, name) => nameSegments(qual) :+ name
    case _                  => Nil
  }

  /** Is the given tree the one generated by the parser when
   *  creating new xml literals?
   */
  def isXmlLiteral(tree: Tree): Boolean = tree match {
    // Stats may not be null; attributes in xml lead to constructs like
    // { var $md = xml.Null ; $md = new UnprefixedAttribute ; new xml.Elem(...) }
    case Block(_, expr)                              => isXmlLiteral(expr)
    case Apply(Select(New(tpt), nme.CONSTRUCTOR), _) => nameSegments(tpt) == XmlElemNameSegments
    case _                                           => false
  }
