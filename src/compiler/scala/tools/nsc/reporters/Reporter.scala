...
abstract class Reporter extends scala.reflect.internal.Reporter {
  ...
  override def hasErrors: Boolean = (
     (errorInitialized && super.hasErrors) // optimization, avoid allocating ERROR just to check if we have errors.
     || cancelled
  )
  ...
  // partest used to expect this inner class
  // TODO: remove duplication here, and consolidate reflect.internal.{ReporterImpl & ReporterImpl}
  ...
  private var errorInitialized = false
