    ...
    def javaCompile(): Unit = {
      ...
      if (javaSources.nonEmpty) {
        ...
        javaDone.completeWith(Future {
          ...
          def classFilesInTarget = Files.walk(Paths.get(command.settings.outdir.value)).iterator().asScala.filter(_.getFileName.toString.endsWith(".class")).toList
          log("Class files before: " + classFilesInTarget.mkString(", "))
          val fileManager = ToolProvider.getSystemJavaCompiler.getStandardFileManager(null, null, null)
