...
//            if (fun.symbol.isConstructor) Static(true) else SuperCall(mix);
    ...
    def zeroOf(k: TypeKind): Tree = k match {
      ...
      case BYTE            => Literal(0b)
      case SHORT           => Literal(0s)
      case CHAR            => Literal(0.toChar)
      case INT             => Literal(0)
      case LONG            => Literal(0l)
    ...
    def getZeroOf(k: TypeKind): Instruction = k match {
      ...
      case BYTE            => CONSTANT(Constant(0b))
      case SHORT           => CONSTANT(Constant(0s))
      case CHAR            => CONSTANT(Constant(0.toChar))
      case INT             => CONSTANT(Constant(0))
      case LONG            => CONSTANT(Constant(0l))
