  ...
  ****/
  ...
  def lookupImplFor(sym: Symbol, clazz: Symbol): Symbol = {
    ...
    def needsLookup = (
         (clazz != NoSymbol)
      && (clazz != sym.owner)
      && !sym.isEffectivelyFinalOrNotOverriden
      && clazz.isEffectivelyFinalOrNotOverriden
    )
  ...
  class Inliner {
    ...
    def analyzeMethod(m: IMethod): Unit = {
      ...
      def inlineWithoutTFA(inputBlocks: Traversable[BasicBlock], callsites: Function1[BasicBlock, List[opcodes.CALL_METHOD]]): Int = {
        ...
        for(x <- inputBlocks; easyCake = callsites(x); if easyCake.nonEmpty) {
          breakable {
            for(ocm <- easyCake) {
              assert(ocm.method.isEffectivelyFinalOrNotOverriden && ocm.method.owner.isEffectivelyFinalOrNotOverriden)
      ...
      def analyzeInc(i: CALL_METHOD, bb: BasicBlock, receiver: Symbol, stackLength: Int, concreteMethod: Symbol): Boolean = {
        ...
        def isCandidate = (
          ...
          || concreteMethod.isEffectivelyFinalOrNotOverriden
          || receiver.isEffectivelyFinalOrNotOverriden
        )
        ...
        debuglog("Treating " + i
              ...
              + "\n\tconcreteMethod.isEffectivelyFinalOrNotOverriden: " + concreteMethod.isEffectivelyFinalOrNotOverriden)
    ...
    def shouldLoadImplFor(sym: Symbol, receiver: Symbol): Boolean = {
      ...
      def loadCondition = sym.isEffectivelyFinalOrNotOverriden && isMonadicMethod(sym) && isHigherOrderMethod(sym)
