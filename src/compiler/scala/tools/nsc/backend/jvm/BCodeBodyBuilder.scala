...
abstract class BCodeBodyBuilder extends BCodeSkelBuilder {
  ...
  abstract class PlainBodyBuilder(cunit: CompilationUnit) extends PlainSkelBuilder(cunit) {
    ...
    def genCallMethod(method: Symbol, style: InvokeStyle, pos: Position, hostClass0: Symbol = null) {
      ...
      val isTraitSuperCallToObjectMethod =
        style.isSuper && hostSymbol != methodOwner && methodOwner.isTraitOrInterface && ObjectTpe.decl(method.name) != NoSymbol && method.overrideChain.last.owner == ObjectClass
      ...
      ) && !(style.isSuper && hostSymbol != null)) || isTraitSuperCallToObjectMethod
      val receiver = if (useMethodOwner) methodOwner else hostSymbol
      val jowner   = internalName(receiver)

      if (style.isSuper && (isTraitSuperCallToObjectMethod || receiver.isTraitOrInterface) && !cnode.interfaces.contains(jowner))
