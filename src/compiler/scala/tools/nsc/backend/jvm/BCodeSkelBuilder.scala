...
abstract class BCodeSkelBuilder extends BCodeHelpers {
  ...
  abstract class PlainSkelBuilder(cunit: CompilationUnit)
    ...
    def resetMethodBookkeeping(dd: DefDef) {
      ...
      labelDef = ldf.rootResult.map(ld => (ld.symbol -> ld)).toMap
    ...
    def genDefDef(dd: DefDef) {
       ...
       */
      val lds = if (labelDefsAtOrUnder.isEmpty) Nil else labelDefsAtOrUnder(dd.rhs) // OPT fast path for common case
      for(ld <- lds; ldp <- ld.params; if !locals.contains(ldp.symbol)) {
