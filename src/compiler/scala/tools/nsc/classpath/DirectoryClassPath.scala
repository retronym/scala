...
import java.nio.file.{FileSystems, Files, FileVisitOption, SimpleFileVisitor}
  ...
  private[nsc] def packages(inPackage: String): Seq[PackageEntry] = {
    ...
    dirForPackage match {
      ...
      case Some(directory) =>
        ...
        listing.map(f => PackageEntryImpl(prefix + getName(f)))
    }
  }
  ...
  protected def files(inPackage: String): Seq[FileEntryType] = {
    ...
    dirForPackage match {
      ...
      case Some(directory) =>
        ...
        listing.map(f => createFileEntry(toAbstractFile(f)))
    }
  }
