...
trait ScalaSettings extends AbsScalaSettings
  ...
  val source             = ScalaVersionSetting ("-Xsource", "version", "Treat compiler input as Scala source for the specified version, see scala/bug#8126.", initial = ScalaVersion("2.12")).withPostSetHook( setting => {
    _isScala211 = setting.value < ScalaVersion("2.11.0")
    _isScala212 = setting.value < ScalaVersion("2.12.0")
    _isScala213 = setting.value < ScalaVersion("2.13.0")
  })
