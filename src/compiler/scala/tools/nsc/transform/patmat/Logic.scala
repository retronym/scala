...
trait Logic extends Debugging  {
  ...
  trait PropositionalLogic {
    ...
    def simplify(f: Prop): Prop = {
      ...
      def mapConverve[A <: AnyRef](s: Set[A])(f: A => A): Set[A] = {
        var changed = false
        val s1 = s.map {a =>
          val a1 = f(a)
      ...
      def negationNormalFormNot(p: Prop): Prop = p match {
        case And(ops) => Or(mapConverve(ops)(negationNormalFormNot)) // De Morgan
        case Or(ops)  => And(mapConverve(ops)(negationNormalFormNot)) // De Morgan
      ...
      def negationNormalForm(p: Prop): Prop = p match {
        case And(ops)     =>
          val ops1 = mapConverve(ops)(negationNormalForm)
