...
trait MatchAnalysis extends MatchApproximation {
  ...
  // TODO: model dependencies between variables: if V1 corresponds to (x: List[_]) and V2 is (x.hd), V2 cannot be assigned when V1 = null or V1 = Nil
    ...
    def unreachableCase(prevBinder: Symbol, cases: List[List[TreeMaker]], pt: Type): Option[Int] = {
      val start = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(statistics.patmatAnaReach) else null
      ...
      try {
        ...
        if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(statistics.patmatAnaReach, start)
    ...
    def exhaustive(prevBinder: Symbol, cases: List[List[TreeMaker]], pt: Type): List[String] = if (uncheckableType(prevBinder.info)) Nil else {
      ...
      val start = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(statistics.patmatAnaExhaust) else null
      var backoff = false
      ...
      if (backoff) Nil else {
        ...
        try {
          ...
          if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(statistics.patmatAnaExhaust, start)
          pruned
        } catch {
          case ex: AnalysisBudget.Exception =>
