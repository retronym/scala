...
// ------ The tree transformers --------------------------------------------------------
        ...
        case dd @ DefDef(_, _, _, vparamss0, _, rhs0) =>
          ...
          val flatdd = copyDefDef(dd)(
            ...
            rhs = nonLocalReturnKeys get dd.symbol match {
              case Some(k) => atPos(rhs0.pos)(nonLocalReturnTry(newRhs, k, dd.symbol))
              case None    => newRhs
            }
          )
    ...
    private object dependentParamTypeErasure {
      ...
      final case class Unchanged(param: ValDef) extends Widening
      final case class Widened(param: ValDef, tempVal: ValDef) extends Widening
      ...
      def erase(dd: DefDef): (List[List[ValDef]], Tree) = {
        ...
        def eraseDependentType(declaredType: Type): Type = {
          existentialAbstraction(vparamSyms, declaredType) match {
            case ExistentialType(tparams, result) =>
              result.subst(tparams, tparams map (_.info))
            case tp => tp
          }
        }
        ...
        val widenings: List[Widening] =
          vparamss.flatten.map { p =>
            ...
            val widened = eraseDependentType(declaredType)
            if (widened =:= declaredType) Unchanged(p)
            else {
              // Change the type of the param symbol
              p.symbol updateInfo widened

              // Create a new param tree
              val newParam: ValDef = copyValDef(p)(tpt = TypeTree(widened))
              ...
              Widened(newParam, tempVal)
            }
          }

        val allParams = widenings map (_.param)
        val (widenedParams, tempVals) = widenings.collect {
          case Widened(param, tempVal) => (param, tempVal)
        }.unzip

        val rhs1 = localTyper.typedPos(rhs.pos) {
          // Patch the method body to refer to the temp vals
          val rhsSubstituted = rhs.substituteSymbols(widenedParams map (_.symbol), tempVals map (_.symbol))
