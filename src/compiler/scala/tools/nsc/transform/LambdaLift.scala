...
abstract class LambdaLift extends InfoTransform {
  ...
  class LambdaLifter(unit: CompilationUnit) extends explicitOuter.OuterPathTransformer(unit) {
    ...
    private val origNames = new LinkedHashMap[Symbol, Name]
        ...
        if (enclosure.isPackageClass || !markFree(sym, enclosure.skipConstructor.owner.logicallyEnclosingMember)) false
          ...
          if (!ss(sym)) {
            ...
            beforePickler {
              ...
              //     closure: (x$1: Int)() => Int
              if (sym.isValueParameter && mexists(sym.owner.info.paramss)(_ == sym)) {
                origNames(sym) = sym.name // register this symbol to have its original named restored in transformUnit
              }
            }
...
*/
    ...
    override def transformUnit(unit: CompilationUnit) {
      ...
      afterOwnPhase {
        ...
        // SI-6028 After we have cut the cord between the lifted trees and their free variables,
        //         restore the original names in the tree from which the code was lifted.
        //         Otherwise, we see:
        //
        //         def closure(x: Int) = { () => x }
        //
        ...
        //
        //         This is problematic for named arguments (0e170e4b), and more
        //         generally to tools reflecting on the parameter names in the
        //         generated bytecode (including someone using a debugger.)
        for ((sym, origName) <- origNames)
          sym.name = origName
      }
