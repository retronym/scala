...
abstract class LambdaLift extends InfoTransform {
  ...
  class LambdaLifter(unit: CompilationUnit) extends explicitOuter.OuterPathTransformer(unit) {
    ...
    /** The original names of free variables */
    private val origFreeVarNames = new LinkedHashMap[Symbol, Name]
...
*/
    ...
    override def transformUnit(unit: CompilationUnit) {
      ...
      afterOwnPhase {
        ...
        // After we have cut the cord between the lifted trees and their free variables,
        ...
        for ((sym, origName) <- origFreeVarNames)
          sym.name = origName
      }
