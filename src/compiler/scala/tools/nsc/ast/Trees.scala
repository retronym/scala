...
import scala.tools.util.StringOps.{ indentNextLine, truncateString, ljust }
...
trait Trees extends reflect.internal.Trees { self: Global =>
 ...
 // --- subcomponents --------------------------------------------------
  ...
  override implicit def treeOps[T <: Tree](tree: T): TreeOps[T] = new TreeOps(tree)

  class TreeOps[T <: Tree](tree: T) extends super.TreeOps[T](tree) {
