...
trait Trees extends scala.reflect.internal.Trees { self: Global =>
 ...
 // --- subcomponents --------------------------------------------------
  ...
  class Transformer extends super.InternalTransformer {
  ...
  abstract class BaseTransformer extends Transformer {
    ...
    @inline final override def atOwner[A](owner: Symbol)(trans: => A): A = {
  ...
  private class ResetAttrs(brutally: Boolean, leaveAlone: Tree => Boolean) {
    ...
    class Transformer extends self.BaseTransformer {
      override def transform(tree: Tree): Tree = {
