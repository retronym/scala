            ...
            if (tparams.isEmpty) { // all type params are defined
              def handleMonomorphicCall: Tree = {
                ...
                def isSelectionFromListModule(tree: Tree) = tree match {
                  case treeInfo.Applied(core @ Select(qual, _), _, _) => treeInfo.isQualifierSafeToElide(core) && qual.symbol == ListModule
                  case _ => false
                }
                if (args.isEmpty && canTranslateEmptyListToNil && fun.symbol.isInitialized && ListModule.hasCompleteInfo && (fun.symbol == List_apply && isSelectionFromListModule(tree)))
