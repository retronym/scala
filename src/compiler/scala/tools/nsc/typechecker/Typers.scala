...
trait Typers extends Adaptations with Tags with TypersTracking with PatternTypers {
  ...
  abstract class Typer(context0: Context) extends TyperDiagnostics with Adaptation with Tag with PatternTyper with TyperContextErrors {
    ...
    def silent[T](op: Typer => T,
      ...
      val rawTypeStart = if (Statistics.areSomeColdStatsEnabled) statistics.startCounter(rawTypeFailed) else null
      val findMemberStart = if (Statistics.areSomeColdStatsEnabled) statistics.startCounter(findMemberFailed) else null
      val subtypeStart = if (Statistics.areSomeColdStatsEnabled) statistics.startCounter(subtypeFailed) else null
      val failedSilentStart = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(failedSilentNanos) else null
      def stopStats() = {
        if (Statistics.areSomeColdStatsEnabled) statistics.stopCounter(rawTypeFailed, rawTypeStart)
        if (Statistics.areSomeColdStatsEnabled) statistics.stopCounter(findMemberFailed, findMemberStart)
        if (Statistics.areSomeColdStatsEnabled) statistics.stopCounter(subtypeFailed, subtypeStart)
        if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(failedSilentNanos, failedSilentStart)
      }
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
    ...
    def isCapturedExistential(sym: Symbol) = (
      (sym hasAllFlags EXISTENTIAL | CAPTURED) && {
        val start = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(isReferencedNanos) else null
        try !isReferencedFrom(context, sym)
        finally if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(isReferencedNanos, start)
      }
    )
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def tryTypedApply(fun: Tree, args: List[Tree]): Tree = {
        val start = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(failedApplyNanos) else null

        def onError(typeErrors: Seq[AbsTypeError], warnings: Seq[(Position, String)]): Tree = {
          if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(failedApplyNanos, start)
      ...
      def normalTypedApply(tree: Tree, fun: Tree, args: List[Tree]) = {
        ...
        val appStart = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(failedApplyNanos) else null
        val opeqStart = if (Statistics.areSomeColdStatsEnabled) statistics.startTimer(failedOpEqNanos) else null
        ...
        def onError(error: SilentTypeError): Tree = fun match {
          case Select(qual, name) if isConversionCandidate(qual, name) =>
            ...
            if (treeInfo.isVariableOrGetter(qual1)) {
              if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(failedOpEqNanos, opeqStart)
              ...
              if (Statistics.areSomeColdStatsEnabled) statistics.stopTimer(failedApplyNanos, appStart)
        ...
        silentResult match {
          case SilentResultValue(fun1) =>
            ...
            if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(typedApplyCount)
            val noSecondTry = (
                 isPastTyper
              || context.inSecondTry
      ...
      def typedSelectOrSuperCall(tree: Select) = tree match {
        ...
        case Select(qual, name) =>
          if (name.isTypeName)
            ...
            if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(typedSelectCount)
      ...
      def typedIdentOrWildcard(tree: Ident) = {
        val name = tree.name
        if (Statistics.areSomeColdStatsEnabled) statistics.incCounter(typedIdentCount)
