  ...
  sealed class Context private[typechecker](val tree: Tree, val owner: Symbol, val scope: Scope,
    ...
    def lookupSymbol(name: Name, qualifies: Symbol => Boolean): NameLookup = {
      ...
      if (name == nme.CONSTRUCTOR) return {
        ...
        val iterator = lookupInScopeIterator(scope) // OPT common case is an an empty iterator
      ...
      while (defSym == NoSymbol && (cx ne NoContext) && (cx.scope ne null)) {
        ...
        val iterator = lookupInScopeIterator(cx.scope) // OPT common case is an an empty iterator
        defSym = if (iterator.isEmpty) searchPrefix else (iterator.toList: @unchecked) match {
          case entries @ (hd :: tl) =>
